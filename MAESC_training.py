...
def main(rank, args):
    ...
    args.device=device
    tokenizer = ConditionTokenizer(args=args)       #用于label和sentiment的tokenizer
    ...
    if args.model_config is not None:
        ...
        bart_config = MultiModalBartConfig.from_pretrained(args.checkpoint)  # bart的模型设置
    ...
    print(args.no_train)
    if args.no_train: #预测或验证阶段
        if args.dataset[0][0] == 'twitter15':
            model=torch.load('/data/liuxj/aspect_sentiment_detect/AOM(copy3)/mytrain/good/2024-06-03-11-32-14/AoM.pt').to(device)
        else:
            model=torch.load('/data/liuxj/aspect_sentiment_detect/amycode-itm-objdet/3attention/AOM(7*7)/mytrain/good_17/2024-06-18-11-01-45/AoM.pt').to(device)

        res_test = eval_utils.eval(args, model, train_loader, metric, device)
        ...
        #训练阶段
        #model = torch.load('/data/liuxj/aspect_sentiment_detect/AOM(copy3)/mytrain/good2/2024-06-13-00-57-10/AoM.pt').to(device)

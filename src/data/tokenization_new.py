...
class ConditionTokenizer:
    ...
    def __init__(self,
                 args,
                 pretrained_model_name='bartbase',
                 cls_token="<<cls>>",
                 mlm_token="<<mlm>>",
                 mrm_token="<<mrm>>",
                 trc_token="<<trc>>",
        ...
        self._base_tokenizer.add_tokens(self.additional_special_tokens) #添加特殊额外的token。
    ...
    def encode_condition(self, img_num=None, sentence=None, text_only=False):  #tokenize text, image features and event
        """
        tokenize text, image features and event
        the output format (after decoded back):
        task_type [<img> <img_feat> ... <img_feat> </img>]

        :param task_type: str or list[str]
        :param img_num: int or list[int], the number of image features
        :param sentence: str or list[str],the sentence of list
        ...
        if image_text is not None:
            image_sentence = self.encode(image_text,
                                         ...
                                         return_tensors='pt',
                                         padding=True) #图像ids,<img><img_feat><img_feat><img_feat>...</img>转化为id
            ...
            input_ids = torch.cat((image_ids, input_sentence_tokens), 1) #图像块的id与文本token的id合并
        ...
        encoded['text_input_ids'] = input_sentence_tokens
        encoded['text_attention_mask'] = input_sentence_mask

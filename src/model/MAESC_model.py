...
import os
from src.model.align_model import *
from src.model.aobj_detect import *
...
class MultiModalBartModel_AESC(PretrainedBartModel):
    def build_model(self,
        ...
        if args.bart_init:
            ...
            model.resize_token_embeddings(
                len(tokenizer.unique_no_split_tokens) + num_tokens) #对新加入的tokens添加embedding
        ...
        multimodal_encoder = MultiModalBartEncoder(config, encoder,
                                                   tokenizer.img_feat_id, #获取<img_feat>的id
    ...
    def __init__(self, config: MultiModalBartConfig, args, bart_model,
        ...
        self.relmodel=Coarse2Fine(roberta_name='robertabase', roi_num=49)

        self.obj_detect_net = YOLOV8.from_pretrained(args.checkpoint)
        self.convset_13=self.obj_detect_net.convset_13
        self.detetion_13=self.obj_detect_net.detetion_13
        self.convset_26 = self.obj_detect_net.convset_26
        self.detetion_26 = self.obj_detect_net.detetion_26
        self.convset_52 = self.obj_detect_net.convset_52
        self.detetion_52 = self.obj_detect_net.detetion_52

        #self.objlinear1=nn.Linear(2048,1024)
        #self.objlinear2 = nn.Linear(2048, 768)
        self.objlinear3 = nn.Linear(2048, 1024)

        #self.conv169to49linear = nn.Linear(169, 49)
        self.conv676to49linear = nn.Linear(196, 49)
        self.conv2704to49linear = nn.Linear(784, 49)
        self.gate_linear = nn.Linear(147, 49)
    ...
    def loss_fn(self,output, target, alpha):
        ...
        loss_obj_conf = conf_loss_fn(output_obj[:, 0], target_obj[:, 0]).float()  # 第0个为置信度（IOU值），学习出来的值可能很小，所以有目标直接为1，没有目标为0
        loss_obj_crood = crood_loss_fn(output_obj[:, 1:5], target_obj[:, 1:5]).float()  # 坐标（中心点和宽和高）
        ...
        loss_noobj = conf_loss_fn(output_noobj[:, 0], target_noobj[:, 0]).float()
    ...
    def prepare_state(self,
                      ...
                      o_target_52=None):
        #add resnet
        #imgs_f, img_mean, img_att = self.resnet(image_features)
        #img_att = img_att.view(-1, 2048, 49).permute(0, 2, 1)

        #add 对齐1
        multi_level_feature = self.obj_detect_net(image_features)

        pred_loss, gated_sentence_aware_image1,gated_sentence_aware_image2,gated_sentence_aware_image3,= self.relmodel(
                                           input_ids=si_input_ids, input_mask=sm_input_mask,
                                           img_feat=multi_level_feature,
                                           relation_label=relation_label,
                                           pred_loss_ratio=1.)
        #print(gated_sentence_aware_image1.shape, gated_sentence_aware_image2.shape, gated_sentence_aware_image3.shape)
        #add object_detect
        #ogated_sentence_aware_image1=self.objlinear1(gated_sentence_aware_image1)
        #ogated_sentence_aware_image2 = self.objlinear2(gated_sentence_aware_image2)
        ogated_sentence_aware_image1 = gated_sentence_aware_image1
        ogated_sentence_aware_image2 = gated_sentence_aware_image2
        ogated_sentence_aware_image3 = self.objlinear3(gated_sentence_aware_image3)
        #ogated_sentence_aware_image3 = gated_sentence_aware_image3

        ogated_sentence_aware_image1=ogated_sentence_aware_image1.permute(0,2,1)
        ogated_sentence_aware_image2 = ogated_sentence_aware_image2.permute(0, 2, 1)
        ogated_sentence_aware_image3 = ogated_sentence_aware_image3.permute(0, 2, 1)

        ogated_sentence_aware_image1=ogated_sentence_aware_image1.view((-1,2048,7,7))
        ogated_sentence_aware_image2 = ogated_sentence_aware_image2.view((-1, 2048, 14, 14))
        ogated_sentence_aware_image3 = ogated_sentence_aware_image3.view((-1, 1024, 28, 28))

        convset_out_13 = self.convset_13(ogated_sentence_aware_image1)
        detetion_out_13 = self.detetion_13(convset_out_13)

        convset_out_26 = self.convset_26(ogated_sentence_aware_image2)
        detetion_out_26 = self.detetion_26(convset_out_26)

        convset_out_52 = self.convset_52(ogated_sentence_aware_image3)
        detetion_out_52 = self.detetion_52(convset_out_52)

        o_loss_13 = self.loss_fn(detetion_out_13, o_target_13,
                                0.6)  # a的值0.9，自己训练可以给0.6左右，如果要检测的目标在背景图上占的面积很小需要将a的值给大些，否则召回率较低
        o_loss_26 = self.loss_fn(detetion_out_26, o_target_26, 0.6)
        o_loss_52 = self.loss_fn(detetion_out_52, o_target_52, 0.6)
        o_loss = (o_loss_13 + o_loss_26 + o_loss_52)/6  # 损失求和 #xxxxxxxxxxxxxxxxxxxxxxxxxxxxx
        o_loss=o_loss.float()

        new_loss=(o_loss+pred_loss)/2
        #print(new_loss)
        gated_sentence_aware_image1=gated_sentence_aware_image1.permute(0,2,1)
        gated_sentence_aware_image2 = gated_sentence_aware_image2.permute(0, 2, 1)
        gated_sentence_aware_image3 = gated_sentence_aware_image3.permute(0, 2, 1)

        #gated_sentence_aware_image1=self.conv169to49linear(gated_sentence_aware_image1)
        gated_sentence_aware_image1 = gated_sentence_aware_image1
        gated_sentence_aware_image2 = self.conv676to49linear(gated_sentence_aware_image2)
        gated_sentence_aware_image3 = self.conv2704to49linear(gated_sentence_aware_image3)

        gated_sentence_aware_image=torch.cat([gated_sentence_aware_image1,gated_sentence_aware_image2,gated_sentence_aware_image3],2)
        gated_sentence_aware_image=self.gate_linear(gated_sentence_aware_image)
        gated_sentence_aware_image=gated_sentence_aware_image.permute(0,2,1)

        dict = self.encoder(input_ids=input_ids,
                            image_features=gated_sentence_aware_image,
    ...
    def noun_attention(self,encoder_outputs,noun_embed,mode='multi-head'):
        if mode=='cat':
            ...
            # 多头注意力
            att_features=self.attention(encoder_outputs,noun_embed,noun_embed) #(encoder_output（原子特征）作为查询，noud（候选方面词）作为键值）
    ...
    def multimodal_GCN(self,encoder_outputs,dependency_matrix,attention_mask,noun_mask,sentiment_value=None,threshold=0.8,dropout=0.8):
        ...
        # 填充图像区域情感值
        if self.sentinet_on:
            sentiment_value=nn.ZeroPad2d(padding=(51,0,0,0))(sentiment_value) #图像的情感初始化为0
    ...
    def forward(
        ...
        state,pred_loss = self.prepare_state(input_ids, image_features,noun_mask,
                                   attention_mask,dependency_matrix,sentiment_value,
                                   si_input_ids=si_input_ids,
                                   sm_input_mask=sm_input_mask,
                                   relation_label=relation_label,
                                   o_target_13=o_target_13,
                                   o_target_26=o_target_26,
                                   o_target_52=o_target_52
                                   )
        spans, span_mask = [
        ...
        loss = (self.span_loss_fct(spans[:, 1:], logits, span_mask[:, 1:])+pred_loss)/2

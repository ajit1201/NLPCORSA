...
class MultiModalBartEncoder(nn.Module):
    ...
    def __init__(self, config: MultiModalBartConfig, encoder, img_feat_id,
        ...
        embed_tokens = encoder.embed_tokens  #bart编码器的embeddings
        ...
        self.embed_images = ImageEmbedding(2048, embed_dim) #将原来的2048维度投影到与文本嵌入维度一样
    ...
    def forward(self,
        ...
        inputs_embeds = self._embed_multi_modal(
            input_ids, image_features) * self.embed_scale #文本和图像的嵌入
        embed_pos = self.embed_positions(input_ids) #位置编码

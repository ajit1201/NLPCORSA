...
class SequenceGeneratorModel(nn.Module):
    ...
    def predict(self,
        ...
        :return:
        """
        state,pred_loss = self.seq2seq_model.prepare_state(input_ids, image_features,noun_mask,
        ...
        result = self.generator.generate(
            state,
            tokens=tgt_tokens[:, :3],
            sentiment_value=sentiment_value)  # the prompt is provided to the model
        return result
...
class SequenceGenerator:
    ...
    @torch.no_grad()
    def generate(self, state,tokens=None,sentiment_value=None, gt_tokens=None):
        """
...
def _beam_search_generate(decoder: Seq2SeqDecoder,
    ...
    scores = decoder.decode(tokens=tokens, state=state,sentiment_value=sentiment_value)  # 这里要传入的是整个句子的长度
    # print(scores)
    # print(scores.shape)
    # print(tokens)
    # print(tokens.shape)
    ...
    # print(next_tokens)
    # print(not_eos_mask)
    # print(not_eos_mask.eq(0))
    # print( not_eos_mask.eq(0)[:, :num_beams])
